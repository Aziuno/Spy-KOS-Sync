...
--								["Vulpera"] = L["Vulpera"],
		...
		DataOptions = {
			...
			args = {
				...
				ShareKOSReasons = {
					name = L["ShareKOSReasons"],
					desc = L["ShareKOSReasonsDescription"],
					type = "toggle",
					order = 13,
					width = "full",
					get = function(info)
						return Spy.db.profile.ShareKOSReasons
					end,
					set = function(info, value)
						Spy.db.profile.ShareKOSReasons = value
					end,
				},
				KOSSyncNotifications = {
					name = L["KOSSyncNotifications"],
					desc = L["KOSSyncNotificationsDescription"],
					type = "toggle",
					order = 14,
					width = "full",
					get = function(info)
						return Spy.db.profile.KOSSyncNotifications
					end,
					set = function(info, value)
						Spy.db.profile.KOSSyncNotifications = value
					end,
				},
				KOSSyncDebug = {
					name = L["KOSSyncDebug"],
					desc = L["KOSSyncDebugDescription"],
					type = "toggle",
					order = 15,
					width = "full",
					get = function(info)
						return Spy.db.profile.KOSSyncDebug
					end,
					set = function(info, value)
						Spy.db.profile.KOSSyncDebug = value
					end,
				},
			},
		},
...
Spy.optionsSlash = {
	...
	args = {
		...
		forcesync = {
			name = L["ForceSync"],
			desc = L["ForceSyncDescription"],
			type = 'execute',
			order = 12,
			func = function()
				Spy:ForceFullKOSSync()
			end,
			dialogHidden = true
		},
		sanc = {
...
--		ShowOnlyPvPFlagged=false,
		...
		ShareKOSReasons=true,
		KOSSyncNotifications=true,
		KOSSyncDebug=false,
...
function Spy:CheckDatabase()
	...
	-- KOS Sync data structures
	if not SpyPerCharDB.KOSSyncTimes then
		SpyPerCharDB.KOSSyncTimes = {}  -- Per-guildmate sync times
	end
	-- Migrate old format
	if SpyPerCharDB.KOSSyncTime then
		SpyPerCharDB.KOSSyncTime = nil
	end
	if not SpyPerCharDB.KOSRemovals then
		SpyPerCharDB.KOSRemovals = {}
	end
	-- Migrate existing reason format to new format with attribution
	Spy:MigrateKOSReasons()
...
-- Migrate existing reason format to new format with attribution
function Spy:MigrateKOSReasons()
	if SpyPerCharDB.KOSReasonsMigrated then
		return
	end
	for name, playerData in pairs(SpyPerCharDB.PlayerData) do
		if playerData.reason and type(playerData.reason) == "table" then
			local needsMigration = false
			for reason, value in pairs(playerData.reason) do
				-- Check if this is old format (true or string) rather than new format (table)
			...
			if needsMigration then
				...
				for reason, value in pairs(playerData.reason) do
					if type(value) == "boolean" then
						-- Standard reason (reason = true)
						newReason[reason] = {
							addedBy = Spy.CharacterName,
							addedAt = time(),
							source = "local"
						}
					elseif type(value) == "string" then
						-- Custom reason (reason = "text")
						newReason[reason] = {
							text = value,
							addedBy = Spy.CharacterName,
							addedAt = time(),
					...
					elseif type(value) == "table" then
						-- Already migrated
						newReason[reason] = value
					end
				end
				playerData.reason = newReason
			end
		end
		-- Initialize kosAddedBy if player is on KOS list
		if SpyPerCharDB.KOSData[name] and not playerData.kosAddedBy then
			playerData.kosAddedBy = {}
			playerData.kosAddedBy[Spy.CharacterName] = SpyPerCharDB.KOSData[name]
		end
		-- Initialize guildStats if not present
...
function Spy:ShowConfig()
	-- Use AceConfigDialog to open the config directly
	LibStub("AceConfigDialog-3.0"):Open("Spy")
end
...
function Spy:PlayerEnteringWorldEvent()
	...
	if Spy.ChnlTime > (now - 6) then
		...
		Spy:ZoneChanged()
		-- Request KOS sync from guild after a delay
		if not Spy.KOSSyncRequested then
			Spy.KOSSyncRequested = true
			Spy:ScheduleTimer("RequestKOSSync", 10)
		end
	end
end
...
function Spy:CommReceived(prefix, message, distribution, source)
	if prefix == Spy.Signature and message and source ~= Spy.CharacterName then
		-- Check if this is a KOS sync message
		if strsub(message, 1, 5) == "KOSR|" then
			Spy:HandleKOSSyncMessage(message, source, distribution)
			return
		end
	end
...
-- KOS Sync Message Handler
function Spy:HandleKOSSyncMessage(message, sender, distribution)
	if Spy.db.profile.KOSSyncDebug then
		DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[KOS Sync Debug]|r HandleKOSSyncMessage from "..tostring(sender).." type="..tostring(strsplit("|", message, 3)))
	end
	if not Spy.db.profile.ShareKOSReasons then
		if Spy.db.profile.KOSSyncDebug then
			DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[KOS Sync Debug]|r ShareKOSReasons disabled, ignoring")
		end
		return
	end
	...
	if distribution ~= "GUILD" then
		if Spy.db.profile.KOSSyncDebug then
			DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[KOS Sync Debug]|r Not GUILD distribution, ignoring")
		end
		return
	end
	...
	if Spy.db.profile.KOSSyncDebug then
		DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[KOS Sync Debug]|r Message type: "..tostring(msgType))
	end
	...
	elseif msgType == "RSN" then
		...
		local enemyName = Spy:UnescapeSyncString(parts[3])
		local reasonKey = Spy:UnescapeSyncString(parts[4])
		local reasonText = Spy:UnescapeSyncString(parts[5])
		local timestamp = tonumber(parts[6]) or time()
		Spy:ReceiveKOSReason(sender, enemyName, reasonKey, reasonText, timestamp)
	elseif msgType == "REM" then
		-- KOSR|REM|EnemyName|Timestamp
		local enemyName = Spy:UnescapeSyncString(parts[3])
		local timestamp = tonumber(parts[4]) or time()
		Spy:ReceiveKOSRemoval(sender, enemyName, timestamp)
	elseif msgType == "REQ" then
		-- KOSR|REQ|SenderCharName|FORCE (optional) - someone logged in and wants sync
		local requesterName = parts[3]
		local forceFlag = parts[4]
		-- Get last sync time with this specific person
		local lastSyncWithRequester = (SpyPerCharDB.KOSSyncTimes and SpyPerCharDB.KOSSyncTimes[requesterName]) or 0
		-- If FORCE flag is present, send ALL data (use sinceTime=0)
		if forceFlag == "FORCE" then
			if Spy.db.profile.DebugMode then
				Spy:Print("[KOS Sync] Received FORCE request from "..requesterName..", sending ALL data")
			end
...
function Spy:ReceiveKOSAdd(sender, enemyName, timestamp, reasonKey, reasonText, class, level, race, guild, zone)
	...
	if not playerData.kosAddedBy[sender] then
		playerData.kosAddedBy[sender] = timestamp
	end
	...
	-- If reason was provided, add it
	if reasonKey and reasonKey ~= "" then
		if not playerData.reason then
			playerData.reason = {}
		end
		if not playerData.reason[reasonKey] then
			if reasonKey == L["KOSReasonOther"] and reasonText and reasonText ~= "" then
				playerData.reason[reasonKey] = {
					text = reasonText,
					addedBy = sender,
					addedAt = timestamp,
					source = "sync"
	...
	if Spy.db.profile.KOSSyncNotifications then
		DEFAULT_CHAT_FRAME:AddMessage(L["SpySignatureColored"]..string.format(L["KOSPlayerAdded"], sender, enemyName))
	end
...
-- Receive KOS Reason from guild
function Spy:ReceiveKOSReason(sender, enemyName, reasonKey, reasonText, timestamp)
	if not enemyName or enemyName == "" or not reasonKey or reasonKey == "" then return end
	...
	-- Only add if we don't have this reason yet or if it's newer
	local existingReason = playerData.reason[reasonKey]
	if not existingReason or (existingReason.addedAt and existingReason.addedAt < timestamp) then
		if reasonKey == L["KOSReasonOther"] and reasonText and reasonText ~= "" then
			playerData.reason[reasonKey] = {
				text = reasonText,
				addedBy = sender,
		...
		-- Show notification
		if Spy.db.profile.KOSSyncNotifications then
			local displayReason = reasonKey
			if reasonKey == L["KOSReasonOther"] and reasonText then
...
-- Receive KOS Removal notification from guild
function Spy:ReceiveKOSRemoval(sender, enemyName, timestamp)
	if not enemyName or enemyName == "" then return end

	-- Store removal info but don't remove from our list
	if not SpyPerCharDB.KOSRemovals then
		SpyPerCharDB.KOSRemovals = {}
	end
	SpyPerCharDB.KOSRemovals[enemyName] = {
		removedBy = sender,
		removedAt = timestamp
	}
	...
	-- Show notification
	if Spy.db.profile.KOSSyncNotifications then
		DEFAULT_CHAT_FRAME:AddMessage(L["SpySignatureColored"]..string.format(L["KOSRemovalReceived"], sender, enemyName))
	end
...
function Spy:ReceiveKOSPvPUpdate(sender, enemyName, result, wins, losses, timestamp, accountId)
	...
	-- Update or create guild stats for this sender
	if not playerData.guildStats[sender] or (playerData.guildStats[sender].lastUpdate or 0) < timestamp then
	...
	if Spy.db.profile.KOSSyncNotifications then
		if result == "WIN" then
			DEFAULT_CHAT_FRAME:AddMessage(L["SpySignatureColored"]..string.format(L["KOSGuildKill"], sender, enemyName))
		else
			DEFAULT_CHAT_FRAME:AddMessage(L["SpySignatureColored"]..string.format(L["KOSGuildDeath"], sender, enemyName))
		end
	end
...
-- Escape special characters for sync protocol
function Spy:EscapeSyncString(str)
	if not str then return "" end
	str = str:gsub("%%", "%%25")  -- Must be first
	str = str:gsub("|", "%%7C")
	str = str:gsub(",", "%%2C")
	str = str:gsub(";", "%%3B")
	str = str:gsub("~", "%%7E")
...
-- Unescape special characters from sync protocol
function Spy:UnescapeSyncString(str)
	if not str then return "" end
	str = str:gsub("%%7C", "|")
	str = str:gsub("%%2C", ",")
	str = str:gsub("%%3B", ";")
	str = str:gsub("%%7E", "~")
	str = str:gsub("%%3A", ":")
	str = str:gsub("%%25", "%%")  -- Must be last
	return str
end

-- Request sync on login (called from OnInitialize after a delay)
function Spy:RequestKOSSync(forceFullSync)
	if Spy.db.profile.KOSSyncDebug then
		DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[KOS Sync Debug]|r RequestKOSSync called, force="..tostring(forceFullSync))
	end
	if not Spy.db.profile.ShareKOSReasons then
		if Spy.db.profile.KOSSyncDebug then
			DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[KOS Sync Debug]|r ShareKOSReasons disabled, aborting")
		end
		return
	end
	if not GetGuildInfo("player") then
		if Spy.db.profile.KOSSyncDebug then
			DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[KOS Sync Debug]|r No guild, aborting")
		end
		return
	end

	-- Broadcast our presence - others will send us their data
	local message = "KOSR|REQ|"..Spy.CharacterName
...
-- Manual sync command
function Spy:ManualKOSSync()
	if not Spy.db.profile.ShareKOSReasons then
		DEFAULT_CHAT_FRAME:AddMessage(L["SpySignatureColored"]..L["KOSSyncDisabled"])
		return
	end
	if not GetGuildInfo("player") then
		DEFAULT_CHAT_FRAME:AddMessage(L["SpySignatureColored"]..L["KOSSyncNoGuild"])
		return
	end
	Spy.KOSSyncRequested = false  -- Allow re-requesting
	Spy:RequestKOSSync()
end

-- Force full sync - clears all sync times to get complete data from everyone
function Spy:ForceFullKOSSync()
	if Spy.db.profile.KOSSyncDebug then
		DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[KOS Sync Debug]|r ForceFullKOSSync called")
	end
	if not Spy.db.profile.ShareKOSReasons then
		if Spy.db.profile.KOSSyncDebug then
			DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[KOS Sync Debug]|r ShareKOSReasons is disabled")
		end
		DEFAULT_CHAT_FRAME:AddMessage(L["SpySignatureColored"]..L["KOSSyncDisabled"])
		return
	end
	if not GetGuildInfo("player") then
		if Spy.db.profile.KOSSyncDebug then
			DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[KOS Sync Debug]|r Not in a guild")
		end
		DEFAULT_CHAT_FRAME:AddMessage(L["SpySignatureColored"]..L["KOSSyncNoGuild"])
		return
	end
	if Spy.db.profile.KOSSyncDebug then
		DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[KOS Sync Debug]|r Clearing sync times and requesting sync")
	end
	-- Clear all sync times to force full sync
	SpyPerCharDB.KOSSyncTimes = {}
	DEFAULT_CHAT_FRAME:AddMessage(L["SpySignatureColored"]..L["KOSForceSyncStarted"])
	Spy.KOSSyncRequested = false
	Spy:RequestKOSSync(true)  -- Pass true to indicate force full sync
end

-- Send sync response to a specific player
function Spy:SendKOSSyncResponse(target, sinceTime)
	if not Spy.db.profile.ShareKOSReasons then return end
	if not target then return end

	-- Collect KOS data newer than sinceTime (when we last synced with this person)
	local syncData = {}
	for name, playerData in pairs(SpyPerCharDB.PlayerData) do
		if SpyPerCharDB.KOSData[name] then
			...
			-- Check if player was added to KOS after sinceTime
			if SpyPerCharDB.KOSData[name] > sinceTime then
				include = true
				entry.added = SpyPerCharDB.KOSData[name]
			end

			-- Check for reasons added after sinceTime
			if playerData.reason then
				for reasonKey, reasonData in pairs(playerData.reason) do
					if type(reasonData) == "table" and reasonData.addedAt and reasonData.addedAt > sinceTime then
						if not entry.reasons then entry.reasons = {} end
						entry.reasons[reasonKey] = reasonData
						include = true
					end
				end
			end

			-- Include guild stats if updated after sinceTime
			if playerData.guildStats then
				for guildMember, stats in pairs(playerData.guildStats) do
					if stats.lastUpdate and stats.lastUpdate > sinceTime then
						if not entry.guildStats then entry.guildStats = {} end
						entry.guildStats[guildMember] = stats
						include = true
					end
				end
			end

			-- Check for kosAddedBy entries after sinceTime (so new adders propagate even if nothing else changed)
			if playerData.kosAddedBy then
				for adderName, adderTimestamp in pairs(playerData.kosAddedBy) do
					if adderTimestamp > sinceTime then
						include = true
						break
					end
				end
			...
			-- Always include kosAddedBy if we're including this entry (to propagate original adders)
			if include and playerData.kosAddedBy then
				entry.kosAddedBy = playerData.kosAddedBy
			end

			-- Include player info (class, level, race, guild, lastSeen) if we have it
			if include then
				if playerData.class then entry.class = playerData.class end
				if playerData.level then entry.level = playerData.level end
				if playerData.race then entry.race = playerData.race end
				if playerData.guild then entry.guild = playerData.guild end
				if playerData.time then entry.lastSeen = playerData.time end
				-- Include sender's personal wins/loses against this enemy
				if playerData.wins or playerData.loses then
					entry.senderWins = playerData.wins or 0
					entry.senderLoses = playerData.loses or 0
				end
			end

			if include then
				table.insert(syncData, entry)
			end
		end
	end

	-- Serialize and send (in chunks if needed)
	if #syncData > 0 then
		local serialized = Spy:SerializeKOSSyncData(syncData)
		local message = "KOSR|RSP|"..serialized
		if strlen(message) < 250 then
			Spy:SendCommMessage(Spy.Signature, message, "GUILD")
		else
	...
	-- Update when we last synced with this person (they will have our data now)
	if not SpyPerCharDB.KOSSyncTimes then
		SpyPerCharDB.KOSSyncTimes = {}
	end
	SpyPerCharDB.KOSSyncTimes[target] = time()
end

-- Serialize KOS sync data (simple format)
-- Format: name:addedTime;R~reasonKey~addedBy~addedAt~text;G~guildMember~wins~losses~lastUpdate;A~adderName~timestamp,...
function Spy:SerializeKOSSyncData(data)
	local parts = {}
	for _, entry in ipairs(data) do
		local entryStr = Spy:EscapeSyncString(entry.name)
		if entry.added then
			entryStr = entryStr..":"..entry.added
		else
			entryStr = entryStr..":0"
		end
		-- Serialize reasons (prefixed with R~)
		if entry.reasons then
			for reasonKey, reasonData in pairs(entry.reasons) do
				local escapedKey = Spy:EscapeSyncString(reasonKey)
				local escapedBy = Spy:EscapeSyncString(reasonData.addedBy or "")
				local reasonStr = "R~"..escapedKey.."~"..escapedBy.."~"..(reasonData.addedAt or 0)
				if reasonData.text then
					reasonStr = reasonStr.."~"..Spy:EscapeSyncString(reasonData.text)
				end
				entryStr = entryStr..";"..reasonStr
			end
		end
		-- Serialize guildStats (prefixed with G~)
		...
		-- Serialize kosAddedBy (prefixed with A~)
		if entry.kosAddedBy then
			for adderName, timestamp in pairs(entry.kosAddedBy) do
				local escapedAdder = Spy:EscapeSyncString(adderName)
				local adderStr = "A~"..escapedAdder.."~"..timestamp
				entryStr = entryStr..";"..adderStr
			end
		end
		-- Serialize player info (prefixed with P~): P~class~level~race~guild~lastSeen
		if entry.class or entry.level or entry.race or entry.guild or entry.lastSeen then
			local escapedClass = Spy:EscapeSyncString(entry.class or "")
			local escapedRace = Spy:EscapeSyncString(entry.race or "")
			local escapedGuild = Spy:EscapeSyncString(entry.guild or "")
...
-- Send KOS sync data in chunks
function Spy:SendKOSSyncChunks(syncData)
	local chunkSize = 10
	local chunks = {}
	for i = 1, #syncData, chunkSize do
		local chunk = {}
		for j = i, math.min(i + chunkSize - 1, #syncData) do
			table.insert(chunk, syncData[j])
		end
		table.insert(chunks, chunk)
	end

	-- Send chunks with 1 second delay
	for i, chunk in ipairs(chunks) do
		Spy:ScheduleTimer(function()
			local serialized = Spy:SerializeKOSSyncData(chunk)
			local message = "KOSR|RSP|"..serialized
			Spy:SendCommMessage(Spy.Signature, message, "GUILD")
		end, i)
	end
...
-- Format: name:addedTime;R~reasonKey~addedBy~addedAt~text;G~guildMember~wins~losses~lastUpdate,...
function Spy:ProcessKOSSyncData(sender, data)
	if Spy.db.profile.KOSSyncDebug then
		DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[KOS Sync Debug]|r ProcessKOSSyncData from "..tostring(sender).." datalen="..tostring(data and strlen(data) or 0))
	end
	if not data or data == "" then
		if Spy.db.profile.KOSSyncDebug then
			DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[KOS Sync Debug]|r No data, returning")
		end
		return
	end

	local entriesProcessed = 0
	local entries = {strsplit(",", data)}
	if Spy.db.profile.KOSSyncDebug then
		DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[KOS Sync Debug]|r Processing "..#entries.." entries")
	end
	for _, entryStr in ipairs(entries) do
		local parts = {strsplit(";", entryStr)}
		local nameAndTime = parts[1]
		local nameParts = {strsplit(":", nameAndTime)}
		local name = Spy:UnescapeSyncString(nameParts[1])
		local addedTime = tonumber(nameParts[2]) or 0

		if name and name ~= "" then
			entriesProcessed = entriesProcessed + 1
			if Spy.db.profile.KOSSyncDebug then
				DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[KOS Sync Debug]|r Processing entry: "..name)
			end
			-- Create player data if doesn't exist
			local playerData = SpyPerCharDB.PlayerData[name]
			if not playerData then
				if Spy.db.profile.KOSSyncDebug then
					DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[KOS Sync Debug]|r Creating PlayerData for: "..name)
				end
				playerData = Spy:AddPlayerData(name, nil, nil, nil, nil, true, true)
			end

			-- Initialize tables (kosAddedBy will be populated from A~ entries)
			if not playerData.kosAddedBy then
				playerData.kosAddedBy = {}
			end
			...
			for i = 2, #parts do
				local dataParts = {strsplit("~", parts[i])}
				local dataType = dataParts[1]

				if dataType == "R" then
					-- Reason data: R~reasonKey~addedBy~addedAt~text
					local reasonKey = Spy:UnescapeSyncString(dataParts[2])
					local addedBy = Spy:UnescapeSyncString(dataParts[3])
					local addedAt = tonumber(dataParts[4]) or 0
					local reasonText = Spy:UnescapeSyncString(dataParts[5])

					if reasonKey and reasonKey ~= "" then
						local existingReason = playerData.reason[reasonKey]
						if not existingReason or (existingReason.addedAt and existingReason.addedAt < addedAt) then
				...
				elseif dataType == "G" then
					...
					if guildMember and guildMember ~= "" then
						...
						if not existingStats or (existingStats.lastUpdate or 0) < lastUpdate then
				...
				elseif dataType == "A" then
					-- KOS adder data: A~adderName~timestamp
					local adderName = Spy:UnescapeSyncString(dataParts[2])
					local adderTimestamp = tonumber(dataParts[3]) or 0

					if adderName and adderName ~= "" then
						-- Merge adders - keep earliest timestamp for each adder
						if not playerData.kosAddedBy[adderName] or playerData.kosAddedBy[adderName] > adderTimestamp then
							playerData.kosAddedBy[adderName] = adderTimestamp
						end
					end
				elseif dataType == "P" then
					-- Player info: P~class~level~race~guild~lastSeen
					local syncClass = Spy:UnescapeSyncString(dataParts[2])
					local syncLevel = tonumber(dataParts[3]) or 0
					local syncRace = Spy:UnescapeSyncString(dataParts[4])
					local syncGuild = Spy:UnescapeSyncString(dataParts[5])
					local syncLastSeen = tonumber(dataParts[6]) or 0

					-- Only update if we don't have the data or synced data is more complete
					if syncClass and syncClass ~= "" and not playerData.class then
						playerData.class = syncClass
					end
					if syncLevel and syncLevel > 0 and (not playerData.level or playerData.level == 0) then
						playerData.level = syncLevel
					end
					if syncRace and syncRace ~= "" and not playerData.race then
						playerData.race = syncRace
					end
					if syncGuild and syncGuild ~= "" and not playerData.guild then
						playerData.guild = syncGuild
					end
					-- Update last seen time if synced time is more recent
					if syncLastSeen > 0 and (not playerData.time or syncLastSeen > playerData.time) then
				...
				elseif dataType == "W" then
					...
					if senderWins > 0 or senderLoses > 0 then
						...
						-- Store/update sender's record in guildStats
						local existingStats = playerData.guildStats[sender]
						if not existingStats or (existingStats.wins or 0) + (existingStats.losses or 0) < senderWins + senderLoses then
							playerData.guildStats[sender] = {
								wins = senderWins,
								losses = senderLoses,
								lastUpdate = time(),
					...
					-- Legacy format (no prefix): reasonKey~addedBy~addedAt~text
					local reasonKey = Spy:UnescapeSyncString(dataParts[1])
					local addedBy = Spy:UnescapeSyncString(dataParts[2])
					local addedAt = tonumber(dataParts[3]) or 0
					local reasonText = Spy:UnescapeSyncString(dataParts[4])

					if reasonKey and reasonKey ~= "" and reasonKey ~= "R" and reasonKey ~= "G" and reasonKey ~= "A" and reasonKey ~= "P" and reasonKey ~= "W" then
						local existingReason = playerData.reason[reasonKey]
						if not existingReason or (existingReason.addedAt and existingReason.addedAt < addedAt) then
							if reasonText and reasonText ~= "" then
								playerData.reason[reasonKey] = {
			...
			-- Add player to KOSData since they came from KOS sync (sync only includes KOS players)
			if not SpyPerCharDB.KOSData[name] then
				local kosTime = addedTime
				if kosTime == 0 or kosTime == nil then
					if Spy.db.profile.KOSSyncDebug then
						DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[KOS Sync Debug]|r KOS time for "..name.." was 0, using current time")
					end
					kosTime = time()
				else
					if Spy.db.profile.KOSSyncDebug then
						DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[KOS Sync Debug]|r Using sender's KOS time for "..name..": "..date("%Y-%m-%d %H:%M", kosTime))
					end
				end
				SpyPerCharDB.KOSData[name] = kosTime
				-- Track sender as source if no other adders
				if not next(playerData.kosAddedBy) then
					playerData.kosAddedBy[sender] = kosTime
				end
			end
			-- Set kos flag on PlayerData so it shows in Stats window KOS filter
			playerData.kos = 1
		end
	end

	-- Update last sync time with this sender
	if not SpyPerCharDB.KOSSyncTimes then
		SpyPerCharDB.KOSSyncTimes = {}
	end
