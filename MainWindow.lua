		...
		elseif level == 3 then
			...
			if key == L["KOSReasonDropDownMenu"] then
				for v, reason in pairs(Spy_KOSReasonList[index].content) do
					...
					info.checked = nil
					if playerData and playerData.reason and playerData.reason[reason] then
						-- Check for both old format (true) and new format (table)
						if playerData.reason[reason] == true or type(playerData.reason[reason]) == "table" then
							info.checked = true
						end
					end
...
function Spy:SetBar(num, name, desc, value, colorgroup, colorclass, tooltipData, opacity)
	...
	-- Set the target macro for click-to-target functionality
...
function Spy:ShowTooltip(self, show, id)
	if show then
		...
		local name = Spy.ButtonName[self.id] or (unit and unit.name)
			...
			if playerData then
				...
				if SpyPerCharDB.KOSData[name] then
					...
					if playerData.reason and Spy.db.profile.DisplayKOSReason then
						for reason, reasonData in pairs(playerData.reason) do
							local displayText = ""
							local addedByText = ""
							-- Handle new format (table with attribution)
							if type(reasonData) == "table" then
								if reason == L["KOSReasonOther"] and reasonData.text then
									displayText = reasonData.text
								else
									displayText = reason
								end
								if reasonData.addedBy then
									addedByText = " |cff888888("..L["KOSReasonAddedBy"].." "..reasonData.addedBy..")|r"
								end
							-- Handle old format (true or string)
							elseif type(reasonData) == "boolean" then
								displayText = reason
							elseif type(reasonData) == "string" then
								displayText = reasonData
							end
							GameTooltip:AddLine(L["KOSReasonIndent"]..displayText..addedByText, reasonText.r, reasonText.g, reasonText.b)
						end
					end
				end
